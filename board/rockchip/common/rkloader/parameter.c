...
const disk_partition_t* get_disk_partition(const char *name)
{
	...
	if (BOOT_FROM_UMS == media) {
		/* when booting with external USB/SATA harddisk,
		assume HD use GPT partition table*/
		block_dev_desc_t* dev_desc = UMSGetBlockDev();
		for (i=0; i<g_gpt_part_count; i++) {
			disk_partition_t* d = &g_gpt_partition_array[i];
			if (strcmp(d->name, name)==0) {
				printf("TSAI: get_disk_partition UMS name=%s lba=%u +%u @%s\n", name,
						(uint)d->start, (uint)d->size, __FILE__);
				return d;
			}
		}
		return NULL;
	}
	part_index = find_mtd_part(&gBootInfo.cmd_mtd, name);
#else
	...
	printf("TSAI: get_disk_partition MTD name=%s lba=%u +%u @%s\n", name,
			(uint)gBootInfo.cmd_mtd.parts[part_index].start, (uint)gBootInfo.cmd_mtd.parts[part_index].size, __FILE__);
	return &gBootInfo.cmd_mtd.parts[part_index];
}

/* caller: board_late_init() */
...
#if TSAI
	{
		printf("TSAI: load_disk_partitions %s\n", __FILE__);
		uint32 media = StorageGetBootMedia();
		if (BOOT_FROM_UMS == media) {
			/* when booting with external USB/SATA harddisk,
			assume HD use GPT partition table*/
