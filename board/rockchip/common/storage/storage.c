...
int StorageReadLba(uint32 LBA, void *pbuf, uint32 nSec)
{
	int ret = FTL_ERROR;
#if TSAI
	if (tsai_print_read_caller) {
		printf("TSAI: StorageReadLba lba=%u + %u fn=%p lr=%p\n",
				(uint)LBA, (uint)nSec, StorageReadLba, __builtin_return_address(0));
	}
#endif
	if(gpMemFun->ReadLba)
	{
...
int StorageWriteLba(uint32 LBA, void *pbuf, uint32 nSec, uint16 mode)
{
	int ret = FTL_ERROR;
#if TSAI
	if (tsai_print_write_caller) {
		printf("TSAI: StorageWriteLba lba=%u + %u fn=%p lr=%p\n",
				(uint)LBA, (uint)nSec, StorageWriteLba, __builtin_return_address(0));
	}
#endif
	...
	if(gpMemFun->WriteLba)
	{
		...
		printf("TSAI ERROR gpMemFun->WriteLba=%p \n", gpMemFun->WriteLba);
	}
#endif
