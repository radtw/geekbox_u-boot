...
printf("TSAI rk_usb_host_lookup %s\n", __FILE__);
...
/* if found UMS, but also detect user has pressed a key, then give up UMS */
int tsai_DetectKeyAbortUMS(void) {
...
#if TSAI
uint32_t g_tsai_gpt_header[512];
#endif
...
#if 0
printf("TSAI: return -1 on purpose, once confirm usb_init() can succeed, remove this line %s\n", __FILE__);
return -1;
#endif
	...
	if (usb_init() >= 0) {
		...
		if (usb_stor_curr_dev >= 0) {
#if TSAI && defined(TSAI_UMS_BOOT_GPT)
			...
			printf("LBA 1 at this disk first 4 byte %08x \n", g_tsai_gpt_header[0]);
			p_gpt_hdr = UMSGetGPTHeader();
			if (p_gpt_hdr->signature == GPT_SIGNATURE) {
			...
			printf("TSAI: data on USB storage at UMS_BOOT_PART_OFFSET, first 4 byte is %08x @%s %d\n", gIdDataBuf[0], __FILE__, __LINE__);
...
#if TSAI
uint32 UMSWriteLBA(uint8 ChipSel, uint32 LBA, void *pbuf, uint32 nSec, uint32 mode)
{
	/* Currently, support one UMS device */
	if ((usb_stor_curr_dev < 0) && (ChipSel != usb_stor_curr_dev))
		return ERROR;
	if(__UMSWriteLBA(usb_stor_curr_dev, LBA, pbuf, nSec) != 0)
		return ERROR;
	return 0;
}
...
#if TSAI
	printf("UMSSysDataLoad UMS_SYS_PART_OFFSET(%x)+lba(%x) \n", UMS_SYS_PART_OFFSET, LBA);
#endif
...
block_dev_desc_t* UMSGetBlockDev(void) {
	block_dev_desc_t *stor_dev = 0;
	if (usb_stor_curr_dev >= 0)
		stor_dev = usb_stor_get_dev(usb_stor_curr_dev);
	return stor_dev;
}

gpt_header* UMSGetGPTHeader(void) {
	return (gpt_header*)&g_tsai_gpt_header;
}

/*
 * GPT header and partition table use 34 sectors, so first partition must be after LBA34
 *
 *
 */
