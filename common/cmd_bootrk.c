 ...
 */
const char* tsai_boot_media_id_string(uint32_t media) {
	char *medianame = NULL;
...
const char* tsai_boot_media_string(void) {
		...
		bootm_headers_t *pimage)
{
	...
	printf("TSAI: rk_load_image_from_ram, ram_addr(string)=%s \n", ram_addr);
...
#endif
	printf("TSAI: rk_load_image_from_storage %s lba=%u, blocks=%u\n", ptn->name, (uint)ptn->start, (uint)ptn->size);
...
#if TSAI
	printf("rk_commandline_setenv bootargs=");
	puts(command_line);
	puts("\n");
#endif
